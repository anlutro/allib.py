...
def popen(command, env=None, copy_env=True, **kwargs):
    ...
    return subprocess.Popen(
        command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=proc_env, **kwargs
    )


def get_result(
    proc, timeout=None, check=False, input=None
    ...
    if check and retcode > 0:
        raise CalledProcessError(retcode, proc.args, output=stdout, stderr=stderr)
